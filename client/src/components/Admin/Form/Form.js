...
const Form = props => {
    ...
    let showtimesElt;
    useEffect(() => {
        showtimesElt = showtimesArray.map((showtime, index) => {
    ...
    useEffect(() => {
        const transferShowtime = () => {
            props.addShowtime(showtimesArray);
        };
        transferShowtime();
    }, [showtimesArray]);
    ...
    const onPosterChange = e => {
        props.onPosterChange(e.target.files[0]);
    };

    const onSnapChange = e => {
        props.onSnapChange(e.target.files[0]);
    };
